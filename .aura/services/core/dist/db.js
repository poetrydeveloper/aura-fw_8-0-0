...
class MemgraphDatabase {
    ...
    async initConstraints() {
        ...
        try {
            // Создаем составную уникальность для семантического паспорта ракушки
            await session.run(`
                CREATE CONSTRAINT ON (s:Shell) 
                ASSERT (s.subject, s.verb, s.object) IS UNIQUE
            `);
            console.log('[Memgraph DB] Семантические ограничения успешно применены.');
        }
        catch (error) {
     ...
     */
    async syncShellWithMutation(payload) {
        const session = this.driver.session();
        try {
...
exports.db = new MemgraphDatabase();
